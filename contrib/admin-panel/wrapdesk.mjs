...
export const CLOSED_FILE = '/etc/pcoin/wrapdesk-closed';
...
export function wrapdeskState() {
  ...
  try {
    if (!existsSync(CLOSED_FILE)) return { open: true, note: '' };
    return { open: false, note: readFileSync(CLOSED_FILE, 'utf8') };
  } catch (e) {
    return { open: null, note: '', error: e.message };
  }
}

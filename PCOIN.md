...
| `src/chainparamsseeds.h` | Rewritten: Bitcoin's ~2031 mainnet seeds replaced by PCoin's own BIP155-encoded fixed seeds (35.239.156.16, 178.105.3.51, 152.53.171.190 and its IPv6, all :9444; 35.238.47.14 was removed when that box was destroyed on 2026-08-22). The file **is** `#include`d by `kernel/chainparams.cpp` and mainnet loads it into `vFixedSeeds` as a fallback for when DNS seeding yields nothing; only the test networks and regtest call `vFixedSeeds.clear()`. Regenerate from `contrib/seeds/nodes_main.txt` via `contrib/seeds/generate-seeds.py`. |
...
**`seed.pc.am`** — currently resolving to three public nodes (`35.239.156.16`, `178.105.3.51`, `152.53.171.190`), the same three that ship as compiled-in fixed seeds. Re-confirm the live record with `dig +short seed.pc.am` before editing, since DNS state is not settled by the source tree. On startup, when a

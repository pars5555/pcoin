...
bool AppInitMain(NodeContext& node, interfaces::BlockAndHeaderTipInfo* tip_info)
{
    ...
    // PCoin: eagerly initialize RandomX proof-of-work verification (the
    // ~256 MiB light-mode cache plus a fallback VM) before any header is
    // validated, so an allocation failure on a memory-constrained host is a
    // clean startup error instead of an uncaught exception thrown from the
    // first header validation (e.g. in the msghand thread).
    {
        std::string randomx_error;
        if (!RandomXPowInit(randomx_error)) {

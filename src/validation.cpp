...
static bool CheckBlockHeader(const CBlockHeader& block, BlockValidationState& state, const Consensus::Params& consensusParams, bool fCheckPOW = true)
{
    ...
    // PCoin: PoW is the RandomX hash of the serialized header, not the
    // (double-SHA256) block hash, so the whole header is passed down.
    if (fCheckPOW && !CheckProofOfWorkRandomX(block, block.nBits, consensusParams))
...
{
    // PCoin: anti-DoS pre-check for headers messages. Each header costs one
    // light-mode RandomX hash (~ms) instead of a SHA256d; acceptable because
    // a peer must still present headers with valid commitments/work and the
    // 2000-header batch bound caps the per-message cost. Callers should skip
    // headers that are already in the block index before calling this (see
    // PeerManagerImpl::CheckHeadersPoW), so replaying known-valid headers
    // does not repeatedly burn RandomX time.
    return std::all_of(headers.cbegin(), headers.cend(),
            [&](const auto& header) { return CheckProofOfWorkRandomX(header, header.nBits, consensusParams);});
}

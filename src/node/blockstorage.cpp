    ...
    while (pcursor->Valid()) {
        ...
        if (pcursor->GetKey(key) && key.first == DB_BLOCK_INDEX) {
            ...
            if (pcursor->GetValue(diskindex)) {
                ...
                // PCoin: upstream re-verifies PoW for every stored header
                // here, which is ~free for SHA256d but costs ~1-10ms per
                // header with light-mode RandomX — O(chain height) at every
                // startup would become minutes-to-hours as the chain grows.
                // Every header in this database already passed full RandomX
                // validation before being written (this check only guards
                // against local DB corruption, not adversarial input), so:
                // - cheaply sanity-check nBits for every entry, and
                // - fully re-verify RandomX PoW on a bounded sample (one
                //   header per POW_SPOT_CHECK_INTERVAL of height, incl.
                //   genesis), keeping startup cost negligible while still
                //   catching systematic corruption.
                constexpr int POW_SPOT_CHECK_INTERVAL{1000};
                if (!DeriveTarget(pindexNew->nBits, consensusParams.powLimit)) {
                    LogError("%s: invalid nBits in block index: %s\n", __func__, pindexNew->ToString());
                    return false;
                }
                if (pindexNew->nHeight % POW_SPOT_CHECK_INTERVAL == 0 &&
                    !CheckProofOfWorkRandomX(pindexNew->GetBlockHeader(), pindexNew->nBits, consensusParams)) {
...
bool BlockManager::ReadBlock(CBlock& block, const FlatFilePos& pos) const
{
    ...
    // Check the header
    // PCoin: verify RandomX PoW (the block hash itself no longer proves work).
    if (!CheckProofOfWorkRandomX(block, block.nBits, GetConsensus())) {

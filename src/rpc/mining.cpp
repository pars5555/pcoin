...
static bool GenerateBlock(ChainstateManager& chainman, CBlock&& block, uint64_t& max_tries, std::shared_ptr<const CBlock>& block_out, bool process_new_block)
{
    ...
    // PCoin: grind the nonce against the RandomX PoW hash. This makes
    // generatetoaddress/generateblock the built-in CPU miner; each try is one
    // light-mode RandomX hash.
    while (max_tries > 0 && block.nNonce < std::numeric_limits<uint32_t>::max() && !CheckProofOfWorkRandomX(block, block.nBits, chainman.GetConsensus()) && !chainman.m_interrupt) {
        ++block.nNonce;
        --max_tries;
    }

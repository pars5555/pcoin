...
const char * const BITCOIN_CONF_FILENAME = "pcoin.conf";
...
{
    // Windows:
    //   old: C:\Users\Username\AppData\Roaming\PCoin
    //   new: C:\Users\Username\AppData\Local\PCoin
    // macOS: ~/Library/Application Support/PCoin
    // Unix-like: ~/.pcoin
#ifdef WIN32
    // Windows
    // Check for existence of datadir in old location and keep it there
    fs::path legacy_path = GetSpecialFolderPath(CSIDL_APPDATA) / "PCoin";
    if (fs::exists(legacy_path)) return legacy_path;

    // Otherwise, fresh installs can start in the new, "proper" location
    return GetSpecialFolderPath(CSIDL_LOCAL_APPDATA) / "PCoin";
#else
    fs::path pathRet;
...
#ifdef __APPLE__
    // macOS
    return pathRet / "Library/Application Support/PCoin";
#else
    // Unix-like
    return pathRet / ".pcoin";
#endif
#endif
}
